...

{
 ...
 //   m_camRotRateX = 3.0f;
	...
	std::string ID = "1";
	m_Camera = new FPSCamera(ID);
	m_Camera->OnWindowSizeChanged(m_ScreenDimensions);
...
void Game::Initialize(HWND window, int width, int height)
{
	...
	// the handle to the current window to save the client area rect
...
void Game::Update(DX::StepTimer const& timer)
{
 ...
 //   {
 //       m_camOrientation.x += m_camRotRateX;
 //   }
 ...
 //   m_view = Matrix::CreateLookAt(m_camPosition, m_camLookAt, Vector3::UnitY);
	m_Camera->HandleInput(m_InputCommands, timer.GetElapsedSeconds(), m_width, m_height);
...
void Game::Render()
{
	...
	for (int i = 0; i < numRenderObjects; i++)
	{
		...
		m_displayList[i].m_model->Draw(context, *m_states, local, m_Camera->GetViewMatrix(), m_Camera->GetProjectionMatrix(), false);	//last variable in draw,  make TRUE for wireframe
...
void Game::SelectTri()
{
	...
	m_pRaycast = CastPickingRay();

	auto triangleList = m_displayChunk.GetIntersectingTriangles(m_pRaycast);
	std::sort(triangleList.begin(), triangleList.end());

	if (!triangleList.empty())
	{
		m_SelectedTriangle = std::make_unique<TriangleData>(triangleList.front());
	}
	else
	{
...
int Game::MousePicking()
{
	...
	for (int i = 0; i < m_displayList.size(); i++)
	{
		...
		XMVECTOR nearPoint = XMVector3Unproject(nearSource, 0.0f, 0.0f, m_ScreenDimensions.right, m_ScreenDimensions.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_Camera->GetProjectionMatrix(), m_Camera->GetViewMatrix(), local);

		XMVECTOR farPoint = XMVector3Unproject(farSource, 0.0f, 0.0f, m_ScreenDimensions.right, m_ScreenDimensions.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_Camera->GetProjectionMatrix(), m_Camera->GetViewMatrix(), local);
...
Ray Game::CastPickingRay()
{
	const XMVECTORF32 scale = { 1, 1, 1 };
	XMVECTOR rotate = Quaternion::Identity;
	...
	m_pRaycast.position = nearPoint;
	m_pRaycast.direction = XMVector3Normalize(farPoint - nearPoint);
	return m_pRaycast;
}

void Game::SculptTerrain()
{
	bool change = true;
